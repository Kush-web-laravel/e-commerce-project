                ...
                <form id="updateProductForm" enctype="multipart/form-data">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <select name="category_id" id="categoryName" class="form-control">
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ $product->category_id == $product->category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="sub_category_id" id="subCategoryName" class="form-control">
                                ...
                                @foreach($subcategories as $subcategory)
                                    <option value="{{ $subcategory->id }}" {{ $product->sub_category_id == $product->subCategory->id ? 'selected' : '' }}>
                                        {{ $subcategory->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="productName" name="product_name" value="{{ $product->name }}" autocomplete="off" placeholder="Enter Name">
                            <medium id="productNameErr" class="text-danger"></medium>  
                        </div>
                        <div class="form-group">
                            <label for="productPrice">Price</label>
                            <input type="number" class="form-control" id="productPrice" name="product_price" value="{{ $product->price }}" autocomplete="off" placeholder="Enter Price">
                        ...
                        <div class="form-group">
                            ...
                            <p>Current Image : <img src="{{ asset($product->image) }}" alt="image" width="120" /></p>
                            <medium id="productImageErr" class="text-danger"></medium>
                        </div>
                        <div class="form-group">
                            <label for="productDescription">Description</label>
                            <textarea type="password" class="form-control" id="productDescription" name="product_description" autocomplete="off" placeholder="Enter Description"> {{ $product->description }} </textarea>
    ...
    <script>
        ...
        $(document).ready(function() {
            $('form#updateProductForm').on('submit', function(e){
                    ...
                    if(temp == 0){
                        let url = "{{ route('updateProduct', $product->id) }}";
